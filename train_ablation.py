...
def sil_main(args):
    ...
    for cycle in range(args.num_cycles):
        ...
        if args.no_reconstruct:
            print("\n[A] Local Reconstruction — SKIPPED (ablation)")
            num_imp = 0
            cost_bef = np.mean([matching_cost(g1_list[i], g2_list[i], matching_list[i],
                                              args.node_cost, args.edge_cost)
                                for i in range(len(g1_list))])
            cost_aft = cost_bef
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no_reconstruct', action='store_true',
                        help='消融：跳过 Phase A 局部重构')
